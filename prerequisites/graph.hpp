#ifndef PREREQUISITES_GRAPH_HPP
#define PREREQUISITES_GRAPH_HPP
...
public:
    ...
    void topologicalSortUtil(vertex_iter w, bool visited[], std::stack<int>& Stack);
    std::stack<int> topologicalSort();
    int calculateSemesters(int index);
};
...
Graph::inedge_iter::inedge_iter(Graph& x, vertex_iter& y): myGraph(x), myVertexIter(y), current(nullptr){
    ...
    if(find.current != nullptr){
        while(find.current != nullptr && find.current->v.label != myGraph.listArray[myVertexIter.current].v.label){
...
void Graph::topologicalSortUtil(Graph::vertex_iter w, bool visited[], std::stack<int>& stos) {
    visited[w.current] = true;
    auto i = w.outedges_begin();
    auto e = w.outedges_end();
    while(i!=e){
        if(!visited[i.current->v.vertexNumber]){
            vertex_iter n(*this);
            n.current = i.current->v.vertexNumber;
            topologicalSortUtil(n, visited, stos);
        }
        ++i;
    }
    stos.push(w.current);
...
std::stack<int> Graph::topologicalSort() {
    ...
    auto i = this->vertices_begin();
    auto e = this->vertices_end();
    while(i!=e){
        if (!visited[i.current]){
            topologicalSortUtil(i, visited, stos);
        }
        ++i;
    }
...
int Graph::calculateSemesters(int index) {
    vertex_iter v(*this);
    v.current = index;
    auto i = v.inedges_begin();
    auto e = v.inedges_end();
    if(i.current==nullptr){
        v.parentGraph.listArray[v.current].v.minSemester = 1;
    }else{
        while(i!=e){
            if(this->listArray[i.source().current].v.minSemester >= v.parentGraph.listArray[v.current].v.minSemester){
                v.parentGraph.listArray[v.current].v.minSemester = this->listArray[i.source().current].v.minSemester + 1;
            }
            ++i;
        }
    }
    return v.parentGraph.listArray[v.current].v.minSemester;
}

#endif //PREREQUISITES_GRAPH_HPP
